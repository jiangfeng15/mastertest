...
/**
  ...
  */
int astConfig::LoadIni(char * cIniPath)
{
    int nRet = 0;
    m_strFilePath = cIniPath;
    nRet = read_profile_string("SERVER","ip",m_cIp, 31,NULL);
    if(!nRet)
        return nRet;
    m_nPort = read_profile_int("SERVER","port",11000);

    m_nTimeOut = read_profile_int("SERVER","timeout",3000);

    m_nLogLevel = read_profile_int("LOG","level",0);

    m_nLangId = read_profile_int("LOG","level",2052);
...
int astConfig::read_profile_string( const char *section, const char *key,char *value,
   int size, const char *default_value)
{
    ...
    assert(value != NULL);
    assert(size > 0);
    assert(m_strFilePath.c_str() !=NULL &&strlen(key));

    if(!load_ini_file(m_strFilePath.c_str(),buf,&file_size))
    {
...
*/
int astConfig::read_profile_int( const char *section, const char *key,int default_value)
{
  char value[32] = {0};
...
int astConfig::write_profile_string(const char *section, const char *key, const char *value)
{
    char buf[MAX_FILE_SIZE]={0};
    ...
    out = fopen(m_strFilePath.c_str(),"w");
    if(NULL == out)
    {
       return 0;
    }
