...
QString SystemInfo::getLastUpTime()
{
    QString qsRes;
    static quint8 count = 0;

    if (! count) {
        const QStringList lines = CommandUtil::exec("bash",{"-c", WHOB_COMMAND}).split("\n");
        if (! lines.isEmpty())
        {
            qsRes = lines.first();
            qsRes = qsRes.mid(qsRes.indexOf(QRegExp("\\s+"))+1,-1);
            lastUpTime = qsRes;
        }
    }
    return qsRes;
...
QString SystemInfo::getOsVersion()
{
    ...
    if (! count) {
        const QStringList lines = CommandUtil::exec("bash",{"-c", LSB_RELEASE}).split("\n");
        if (! lines.isEmpty())
        {
            for(QString echoline:lines)
            {
                QStringList qslist = echoline.split(QRegExp("\t"));
                if(!qslist.last().isEmpty())
                {
                    if(qslist.first() == "Release:")
                     {
                        qsRes = qslist.last();
                        osVersion = qsRes;
                    }
                    else if(qslist.first() == "Description:")
                    {
                        osDescription = qslist.last();
                    }
...
QString SystemInfo::getOsLanguage()
{
    QString qsRes;
    static quint8 count = 0;

    if (! count) {
        const QStringList lines = CommandUtil::exec("bash",{"-c", LANG_COMMAND}).split("\n");
        if (! lines.isEmpty())
        {
            qsRes = lines.first();
            osLanguage = qsRes;
...
QString SystemInfo::getOsInstallTime()
{
    QString qsRes;
    static quint8 count = 0;

    if (! count) {
        const QStringList lines = CommandUtil::exec("bash",{"-c", INSTALL_COMMAND}).split("\n");
        if (! lines.isEmpty())
        {
            for (QString echoline:lines)
            {
                QStringList qsList = echoline.split(QRegExp("\\s+"));
                if(qsList.last()=="lost+found")
                {
                    qsRes = QString("%1 %2").arg(qsList.at(5)).arg(qsList.at(6).mid(0,qsList.at(6).indexOf(".")));
                    osInstallTime = qsRes;
                    break;
                }
            }
