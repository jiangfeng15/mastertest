...
bool SQLiteWrapper::Open(std::string const& db_file) {
	...
	//	return false;
	//}
	if (sqlite3_open_v2(db_file.c_str(), &db_, SQLITE_OPEN_READWRITE, NULL) != SQLITE_OK) {
		return false;
	}
	return true;
}
...
bool SQLiteStatement::Execute() {
	...
	if (rc == SQLITE_BUSY) {
		//::MessageBox(0, L"SQLITE_BUSY", 0, 0);
		return false;
	}
	if (rc == SQLITE_ERROR) {
		//::MessageBox(0, L"SQLITE_ERROR", 0, 0);
		return false;
	}
	if (rc == SQLITE_MISUSE) {
		//::MessageBox(0, L"SQLITE_ERROR", 0, 0);
...
bool SQLiteStatement::NextRow() {
	...
	if (rc == SQLITE_DONE) {
		...
		//::MessageBox(0, L"SQLiteStatement::NextRow SQLITE_MISUSE", 0, 0);
	}
	else if (rc == SQLITE_BUSY) {
		//::MessageBox(0, L"SQLiteStatement::NextRow SQLITE_BUSY", 0, 0);
	}
	else if (rc == SQLITE_ERROR) {
		//::MessageBox(0, L"SQLiteStatement::NextRow SQLITE_ERROR", 0, 0);
	}
	return false;
}
